...
namespace TTSIntegration
{
    public class DynamicWrapper
    {
        ...
        public DynamicWrapper(dynamic value)
        {
            Value = value;
        }
    }
    public partial class Form2 : Form
    {
        ...
        private dynamic _pyModule;
        private readonly DynamicWrapper _pyModuleWrapper;
        
        public Form2()
        {
            _modelRunner = new ModelRunner(ref _pyModule);
            _pyModuleWrapper = new DynamicWrapper(_pyModule);
            InitializeComponent();
        ...
        private static async Task ConcatenateAndSave(List<float[]> audioDataList, string path, int sampleRate = 22050)
        {
        ...
        private async void AudioGenerateBtnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                List<float[]> wavData = await _modelRunner.JustFetchAudioArray(_pyModuleWrapper, inputText:text);
                stopwatch.Stop();
                _audioFilePath = $"generated_audio_{_audioCounter}_in_{stopwatch.ElapsedMilliseconds / 1000}_seconds.wav";
                await ConcatenateAndSave(audioDataList: wavData, path: _audioFilePath, sampleRate: 22050); // for vits model
                ...
                this.audioPlayerBtn.Visible = true;
                //todo: need to keep eye on here
                GC.Collect();
                const string caption = "Conversion of Text to Audio Completed";
        ...
        private async void SpeakOutBtnClick(object sender, EventArgs e)
        {
            await Task.Run(() =>
            {
